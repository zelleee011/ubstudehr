...
function populateRecentPatientsWidget() {
    ...
    recent.forEach((p, index) => {
        let status = 'Stable'; let statusClass = 'status-completed'; 
        if(isPatientCritical(p)) { status = 'Critical Alert'; statusClass = 'status-critical'; } 
        else if(p.complaint.toLowerCase().includes('checkup') || p.complaint.toLowerCase().includes('fever')) { status = 'Follow-up'; statusClass = 'status-pending'; }
        ...
        list.innerHTML += `
            <div class="recent-list-item">
                <div class="r-patient-info"><div class="r-avatar">${initials}</div><div><h4>${p.name}</h4><p style="margin-bottom: 2px;">${p.complaint} • Age ${p.age}</p>${assessmentTags}</div></div>
                <div class="r-patient-status"><span class="status-badge ${statusClass}">${status}</span><span class="r-time">${displayTime}</span></div>
            </div>`;
    });
}
...
function savePatient(e) {
    ...
    const newPatient = {
        ...
        allergies: document.getElementById('p-allergies').value || 'None', civil: document.getElementById('p-civil').value, doctor: document.getElementById('p-doc').value,
        complaint: document.getElementById('p-complaint').value, date: document.getElementById('p-admission').value,
        vitals: initialVitals, vitalsHistory: [], record: initialRecord
    };
    if (idField) {
        ...
        patients[index] = newPatient; alert('Patient updated successfully!');
    } else { patients.push(newPatient); alert('Patient saved successfully!'); }
    saveData();
    showSection('patient-records', document.querySelectorAll('.nav-links li')[1]);
}
...
function editPatient(id) {
    ...
    document.getElementById('p-id').value = p.id; document.getElementById('p-name').value = p.name; document.getElementById('p-age').value = p.age;
    document.getElementById('p-sex').value = p.sex; document.getElementById('p-contact').value = p.contact; document.getElementById('p-admission').value = p.date;
    document.getElementById('p-address').value = p.address || ''; document.getElementById('p-blood').value = p.blood; document.getElementById('p-room').value = p.room;
    document.getElementById('p-diet').value = p.diet; document.getElementById('p-allergies').value = p.allergies; document.getElementById('p-doc').value = p.doctor;
...
function populateTable() {
    const tbody = document.getElementById('patients-tbody'); if (!tbody) return; tbody.innerHTML = '';
    patients.forEach(patient => {
        let isRowCritical = isPatientCritical(patient); let rowClass = isRowCritical ? 'critical-row' : '';
        tbody.innerHTML += `
            <tr class="${rowClass}">
                <td>
                    ...
                    <div style="display: inline-block; vertical-align: middle;"><strong style="color: var(--text-dark); font-size: 15px;">${patient.name}</strong><br><span style="font-size: 12px; color: var(--text-light); font-weight: 500;">${patient.age} yrs • ${patient.sex}</span></div>
                </td>
                ...
                <td class="action-cell"><span class="view-btn" onclick="openModal('${patient.id}')">View</span> <i class="fas fa-edit edit-icon" onclick="editPatient('${patient.id}')" title="Edit"></i> <i class="fas fa-trash delete-icon" onclick="deletePatient('${patient.id}')" title="Delete"></i></td>
            </tr>`;
    });
}
...
function populateLabs() {
    ...
    labs.forEach(lab => {
        tbody.innerHTML += `<tr><td><strong style="color:var(--text-dark);">${lab.name}</strong></td><td>${lab.test}</td><td>${lab.date}</td><td><span class="status-badge status-${lab.status}">${lab.status}</span></td><td class="action-cell"><span class="view-btn" style="background:var(--primary-light); color:var(--primary);" onclick="cycleLabStatus(${lab.id})">Update</span> <i class="fas fa-trash delete-icon" onclick="deleteLab(${lab.id})"></i></td></tr>`;
    });
}

function openPharmacyModal() { document.getElementById('new-pharm-name').value = ''; document.getElementById('new-pharm-med').value = ''; document.getElementById('add-pharmacy-modal').style.display = 'flex'; }
...
function populatePharmacy() {
    ...
    pharmacy.forEach(rx => {
        tbody.innerHTML += `<tr><td><strong style="color:var(--text-dark);">${rx.name}</strong></td><td><i class="fas fa-pills" style="color: var(--secondary); margin-right:5px;"></i> ${rx.meds}</td><td>${rx.doc}</td><td><span class="status-badge status-${rx.status}">${rx.status}</span></td><td class="action-cell"><span class="view-btn" style="background:var(--primary-light); color:var(--primary);" onclick="cycleMedStatus(${rx.id})">Update</span> <i class="fas fa-trash delete-icon" onclick="deleteMed(${rx.id})"></i></td></tr>`;
    });
}
...
function openModal(patientId) {
    currentViewedPatientId = patientId; const p = patients.find(x => x.id === patientId); if(!p) return;
    if (!p.vitalsHistory) p.vitalsHistory = []; if (!p.record) p.record = { neuro: '-', skin: '-', bowel: '-', edema: '-', timestamp: '-' };
    document.getElementById('modal-patient-name').innerText = p.name; document.getElementById('modal-patient-info').innerText = `${p.id} • ${p.age} yrs • ${p.sex} • Room: ${p.room}`;
    document.getElementById('modal-doctor').innerText = p.doctor;
    let allergyElem = document.getElementById('modal-allergies'); allergyElem.innerText = p.allergies;
    if(p.allergies !== 'None' && p.allergies !== '') { allergyElem.style.color = '#ff0000'; allergyElem.parentElement.style.background = '#ffe4e6'; allergyElem.parentElement.style.borderColor = '#fda4af'; } 
    else { allergyElem.style.color = 'var(--text-main)'; allergyElem.parentElement.style.background = '#f8fafc'; allergyElem.parentElement.style.borderColor = 'var(--border-color)'; }
    document.getElementById('modal-diet').innerText = p.diet; document.getElementById('modal-complaint').innerText = p.complaint;
    let v = p.vitals || {};
    let bpBad = (v.bpSys !== '-' && (v.bpSys < 90 || v.bpSys > 120)) || (v.bpDia !== '-' && (v.bpDia < 60 || v.bpDia > 80));
    let bpClass = bpBad ? 'red-alert-card' : ''; let hrClass = (v.hr !== '-' && (v.hr < 60 || v.hr > 100)) ? 'red-alert-card' : '';
    let tempClass = (v.temp !== '-' && (v.temp < 36.5 || v.temp > 37.5)) ? 'red-alert-card' : ''; let spo2Class = (v.spo2 !== '-' && v.spo2 < 95) ? 'red-alert-card' : '';
    let respClass = (v.resp !== '-' && (v.resp < 12 || v.resp > 20)) ? 'red-alert-card' : ''; let bmiClass = (v.bmi !== '-' && (v.bmi < 18.5 || v.bmi > 24.9)) ? 'red-alert-card' : '';
    ...
    `;
    document.getElementById('rec-admission').innerText = p.date; document.getElementById('rec-timestamp').innerText = p.record.timestamp || '-';
    const styleAssessmentCard = (elementId, value, normalValues) => {
        const el = document.getElementById(elementId); if(!el) return; el.innerText = value;
        const card = el.parentElement; const label = el.previousElementSibling;
        if (value !== '-' && !normalValues.includes(value)) { card.classList.add('alert-card'); el.classList.add('alert-text'); label.classList.add('alert-text'); } 
        else { card.classList.remove('alert-card'); el.classList.remove('alert-text'); label.classList.remove('alert-text'); }
    };
    styleAssessmentCard('rec-neuro', p.record.neuro, ['Alert']); styleAssessmentCard('rec-skin', p.record.skin, ['Normal', 'Pinkish']);
    styleAssessmentCard('rec-bowel', p.record.bowel, ['Normal Active']); styleAssessmentCard('rec-edema', p.record.edema, ['None (0)']);
    renderChart(p); populateHistoryTable(p);
    document.getElementById('patient-modal').style.display = 'flex'; switchTab('overview'); 
}

function closeModal() { document.getElementById('patient-modal').style.display = 'none'; }

function openRecordModal() {
    if(!currentViewedPatientId) return; const p = patients.find(x => x.id === currentViewedPatientId); if(!p) return;
    let currentTs = (p.record.timestamp && p.record.timestamp !== '-') ? p.record.timestamp : getExactTimestamp();
    document.getElementById('ur-admission').value = p.date || ''; document.getElementById('ur-timestamp').value = currentTs;
    document.getElementById('ur-neuro').value = p.record.neuro; document.getElementById('ur-skin').value = p.record.skin;
    document.getElementById('ur-bowel').value = p.record.bowel; document.getElementById('ur-edema').value = p.record.edema;
...
function savePatientRecord(e) {
    e.preventDefault(); if(!currentViewedPatientId) return; const index = patients.findIndex(p => p.id === currentViewedPatientId); if(index === -1) return;
    patients[index].date = document.getElementById('ur-admission').value;
    patients[index].record = { neuro: document.getElementById('ur-neuro').value, skin: document.getElementById('ur-skin').value, bowel: document.getElementById('ur-bowel').value, edema: document.getElementById('ur-edema').value, timestamp: document.getElementById('ur-timestamp').value };
    saveData(); closeRecordModal(); openModal(currentViewedPatientId); switchTab('record'); 
}
...
function openAddVitalsModal(recordId = null) {
    ...
    if (recordId) {
        ...
        document.getElementById('vitals-modal-title').innerText = "Edit Vitals Record"; document.getElementById('v-id').value = r.id;
        document.getElementById('v-date').value = r.date; document.getElementById('v-bpsys').value = r.bpSys; document.getElementById('v-bpdia').value = r.bpDia;
...
function saveVitals(e) {
    e.preventDefault(); if(!currentViewedPatientId) return; const index = patients.findIndex(p => p.id === currentViewedPatientId); if(index === -1) return;
    let activeTabId = 'vitals'; document.querySelectorAll('.tab-content').forEach(tab => { if(tab.classList.contains('active')) activeTabId = tab.id.replace('tab-', ''); });
    let p = patients[index]; let vId = document.getElementById('v-id').value;
    let newVitalData = { date: document.getElementById('v-date').value, bpSys: parseInt(document.getElementById('v-bpsys').value), bpDia: parseInt(document.getElementById('v-bpdia').value), hr: parseInt(document.getElementById('v-hr').value), temp: parseFloat(document.getElementById('v-temp').value), spo2: parseInt(document.getElementById('v-spo2').value), resp: parseInt(document.getElementById('v-resp').value), bmi: parseFloat(document.getElementById('v-bmi').value) };
    if (vId) { let rIndex = p.vitalsHistory.findIndex(r => r.id == vId); if(rIndex > -1) p.vitalsHistory[rIndex] = { id: parseInt(vId), ...newVitalData }; } 
    else { p.vitalsHistory.push({ id: Date.now(), ...newVitalData }); }
...
async function downloadFromCloud() {
    try {
        ...
        updateSyncStatus("Live 🟢"); return true;
    } catch(e) { console.error("Sync Download Failed", e); updateSyncStatus("Offline 🔴"); return false; }
}

function forceCloudSync() { uploadToCloud(); }
setInterval(async () => { if(isCloudInitialized) { await downloadFromCloud(); } }, 2500);